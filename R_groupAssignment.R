...
vehicle$PriceinGermany<-vehicle$PriceinGermany %>% str_remove("�,�") %>% str_remove(",") %>% str_replace("N/A", "0") %>% as.numeric()
vehicle$PriceinUK<-vehicle$PriceinUK %>% str_remove("£") %>% str_remove(",") %>% str_replace("N/A","0") %>% as.numeric()
...
vehicle <- vehicle %>% rename(`PriceinGermany(�,�)` = PriceinGermany)
vehicle <- vehicle%>% rename(`PriceinUK(£)` = PriceinUK)

vehicle %>% write.csv(file="cheapestElectricCars(Clean).csv")
#1. ????????????????????????????????????????????????????????????????????????????????????????????????????????? select name ????????????
#- highest top speed
...
vehicle %>% select(Name,`PriceinUK(£)`)%>%filter(`PriceinUK(£)`*46.17 == max(`PriceinUK(£)`*46.17,na.rm = T))
#3.???????????????????????????????????????????????????????????????  4 ????????????????????? ???????????????????????????????????????????????????????????????????????????
vehicle %>%select(Name,NumberofSeats,`PriceinUK(£)`) %>%filter(NumberofSeats == 4 & `PriceinUK(£)` < mean(`PriceinUK(£)`,na.rm = T))
#4.?????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
vehicle %>% filter(NumberofSeats == max(NumberofSeats)) %>% count() - vehicle %>% filter(NumberofSeats == min(NumberofSeats)) %>% count()
#5.???????????????????????????????????????????????????????????????????????????????????? ???????????????????????????????????????????????????????????? 150 km/h
vehicle %>%filter(Drive == "Front Wheel Drive" & `TopSpeed(km/h)` > 150) %>% select(Name,`TopSpeed(km/h)`,`PriceinGermany(�,�)`,`PriceinUK(£)`) %>% arrange(desc(`TopSpeed(km/h)`))
...
vehicle %>% filter(Drive == "Front Wheel Drive" &`PriceinUK(£)` == min(frontvehicle$`PriceinUK(£)`,na.rm = T))
vehicle %>% filter(Drive == "All Wheel Drive" & `PriceinUK(£)` == min(allvehicle$`PriceinUK(£)`,na.rm = T))
vehicle %>% filter(Drive == "Rear Wheel Drive" & `PriceinUK(£)` == min(rearvehicle$`PriceinUK(£)`,na.rm = T))
#8.??????????????????????????????????????????????????????n????????????????????????????????? ??????????????????????????????????????????????????????????????????????????????????????????????????????
vehicle %>% filter(`Efficiency(Wh/km)` == max(`Efficiency(Wh/km)`))
#9.????????????????????????????????????????????????????????????????????????????????????????????????
vehicle %>% summarise(mean(`Efficiency(Wh/km)`))
#10.?????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
vehicle %>% filter(`Acceleration(sec)` <= mean(`Acceleration(sec)`)&`TopSpeed(km/h)` >= mean(`TopSpeed(km/h)`)& `Range(km)` >= mean(`Range(km)`)&`Efficiency(Wh/km)`>= mean(`Efficiency(Wh/km)`)&`FastChargeSpeed(km/h)` >= mean(`FastChargeSpeed(km/h)`,na.rm =T)&NumberofSeats >= mean(NumberofSeats)&`PriceinUK(£)` <= mean(`PriceinUK(£)`))
